...
- `uv run ankify sync <path> -v --dry-run` - preview sync
- `uv run ankify sync <path>` - sync to Anki
...
- Tests go in `tests/` mirroring `src/ankify/` structure
...
- Use `TEST_DECK_PREFIX = "ankify-test"` for test decks
